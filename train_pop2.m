...
while ~done
    ...
    if count < 0
        ...
        test_gen = generation > 200;
        %test_gen = 1;
        
        %This code reduces the population size to the best 30 after
        %generation 200
        if generation == 201
...
activations = cam.prepare_robot();
pause(activations + 4); 
if (isnan(distance) || distance > 2700 || distance < -100)
