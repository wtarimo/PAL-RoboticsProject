...
output =[ind1{1} ind1{2} ind1(3) {loop_crossover2(ind1{4},ind2{4},cp1,cp2,0)}];

function loop = loop_crossover2(genes1,genes2,cp1,cp2,count)
if isempty(genes1) || isempty(genes2)
    loop = genes1;
elseif (count <= cp1) || (count > cp2)
    loop = [genes1(1) loop_crossover2(genes1(2:end),genes2(2:end),cp1,cp2,count+1)];
else
    loop = [genes2(1) loop_crossover2(genes1(2:end),genes2(2:end),cp1,cp2,count+1)];
end
...
output = [mutate_gene_prt(gene(1)) mutate_gene_prt(gene(2))];
